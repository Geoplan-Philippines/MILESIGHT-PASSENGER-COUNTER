...
export class CounterPage implements OnInit, OnDestroy {
  ...
  async submit(): Promise<void> {
    ...
    try {
      const authHeader = 'Basic ' + btoa('http@default:j@H3fVT0%n$3NzANWibXV@<F^>RlS297');
      const response = await fetch('https://beaveriot.geoplanph.com/api/v1/workflow-http-in/http@default/passenger-count', {
        method: 'POST',
        headers: {
          'Authorization': authHeader,
          'Content-Type': 'application/json'
        },
        body: this.count().toString()
      });
      ...
      this.showToast('Failed to submit entry', 'error');
    } finally {
